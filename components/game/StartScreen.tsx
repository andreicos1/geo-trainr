...
import { COUNTRY_COVERAGE, getCountriesForContinent } from "@/lib/geo/countries-coverage";
...
export default function StartScreen() {
  ...
  const selectedContinentOrGlobe = scope.type === "continent" ? scope.code : scope.type === "globe" ? "globe" : undefined;

  const highlightedCodes = useMemo(() => {
    if (scope.type === "globe") return new Set(COUNTRY_COVERAGE.map((c) => c.code));
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
      <header className="flex flex-col gap-2">
        <h1 className="text-xl font-bold tracking-tight text-white">geotrainr</h1>
        <p className="text-slate-400">
          Pick a country, a continent, or the whole world. Play 5 rounds of Street View guessing
          against an AI that explains its reasoning.
        </p>
      </header>
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-xs text-slate-500">
          Highlighted countries have Google Street View coverage. Greyed-out countries aren&apos;t
          selectable.
        </p>
      </section>
    </div>
